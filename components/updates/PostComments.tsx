...
export default async function PostComments({ url }: Props) {
	...
	return (
		<>
			<HalftoneDivider className="w-full max-w-screen-md" />
			<div className="flex flex-col items-center justify-center w-full max-w-screen-md gap-2 p-4 text-left border-2 rounded-2xl border-cream-edge/25 bg-zinc-950/75">
				{/* STATS */}
				<HalftoneMarker centered className="mb-1" />
				<h2 className="font-display text-2xl tracking-wide text-center text-cream" id="comments-section">
					Stats
				</h2>
				...
				{!thread.replies || thread.replies.length === 0 ? null : (
					<>
						{/* COMMENTS */}
						<h2 className="font-display text-2xl tracking-wide text-center text-cream">Comments</h2>
						<p className="text-sm">
